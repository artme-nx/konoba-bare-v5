...
10. VIDEO MOMENTI "Četiri kadra" (naslov bez "ognjište"/"ispod peke"; BEZ eyebrowa "Iz kuhinje, uživo" — maknut 6. runda): ČETIRI portretna videa nakon galerije, jednako tretirana, redoslijed vatra → meso → plata → otkrivanje (6. runda: plata i otkrivanje zamijenjeni; H.264 720×1280, bez zvuka, preload=none + WebP posteri). NISU scrubovi: tihi loop preko IntersectionObservera (play u kadru, pause izvan); reduced-motion ili bez IO = kontrole bez autoplaya. Desktop 4 kolone, ≤860px 2×2, ≤767px 1 kolona. Izvorni MOV-ovi u ../SLIKE/video, NE commitaju se.
...
- Jelovnik (4. runda, 2026-07-06, nalog klijenta): naslov samo "Jelovnik"; jedan statični blok u 2 kolone — LIJEVO "Pizze" (14 naziva, "Pišk" IZBAČEN), DESNO gore "Iz konobe": Hladna plata, Dimljena tuna, Slane srdele, Kuhana sabljarka (bez "palamida"), Pršut i sir, Pašteta od tune (preimenovano iz "Marin kruhić s ribljom paštetom"), Salate; DESNO dolje istaknuti blok "Ispod peke" (— Meso / — Hobotnica, na narudžbu dan ranije; 6. runda: blok se flexom razvlači do dna pa donji rub završava TOČNO u liniji s dnom pizza kolone; pizza podnaslov samo "Na terasi uz more"). Naslov galerije: "Iz naše kuhinje" (6. runda). IZBAČENI (ne vraćati): Carpaccio od tune, Marinirani inčuni. Bez cijena, bez animacija, bez slika. Mobitel: pizze → ostala jela → peka.
...
HEADER NA UZAKIM TELEFONIMA (7. runda, 2026-07-06, bugfix): logo+lang-switch+call-mini+hamburger zajedno bili širi od viewporta na ≤~480px (glavni krivac: .logo small nowrap podnaslov ~150px + header gap 26px×3) — hamburger je bio djelomično/potpuno izvan ekrana na sve 3 jezične verzije. Fix: `@media(max-width:640px)` stišće header padding/gap i kompaktira logo-podnaslov + lang-switch (font-size/letter-spacing/gap manji, BEZ mijenjanja stila/veličine same hamburger ikone). Provjereno 320–650px na HR/EN/DE (EN ima najdulji podnaslov "Konoba · Prvić Island" — mjerodavan worst-case). NE povećavati header gap/padding na mobitelu bez ponovne provjere ovih širina.

## Tvrda pravila (Impeccable NE SMIJE pregaziti)
- Mapa: keyless iframe embed, VIDLJIVA ODMAH bez klik-facade (nalog klijenta 2026-07-06; loading="lazy" ostaje) — i dalje NIKAD Maps JS API / key= parametar. Ispod karte .sail-info: rečenica o liniji Vodice–Prvić + gumb "Red plovidbe 2026 (PDF)" (assets/red-plovidbe-2026.pdf, Jadrolinija B-505) + izvor-link; TODO[2027] za zamjenu PDF-a.
